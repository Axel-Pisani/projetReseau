 ...
 * plusieurs thread de lire le fichier en même temps. Si un thread demande une édition du fichier, oneServerMultiClient.comuneCode.FileHandle
...
public class FileHandle {
     ...
     * auparavant.
     * @param file Le fichier qui sera pris en charge par l'objet oneServerMultiClient.comuneCode.FileHandle
     */
    ...
    public OperationStatus readFile(PrintWriter writer) {
        ...
        synchronized (this) {
            try {
                ...
                System.err.println("oneServerMultiClient.comuneCode.FileHandle Interrupted while waiting...");
                e.printStackTrace();
        ...
        synchronized (this) {
            try {
                for(;;)  {
                    ...
                    // If the file is getting deleted, we abort
                    if (state == State.MARKED_FOR_DELETION ) {
                        System.err.println("callableByClient.Write cancelled : file is deleted");
                        return OperationStatus.ERROR_FILE_DELETED;
                    }
    ...
    /**
     * Cette fonction demande à l'objet oneServerMultiClient.comuneCode.FileHandle de supprimer le fichier du disque.
     * Après cette opération, l'objet oneServerMultiClient.comuneCode.FileHandle ne doit plus être utilisé car le fichier correspondant n'existe
     * plus. Il faudra recréer le fichier et créer un nouveau oneServerMultiClient.comuneCode.FileHandle.
     * @return
     * Cette fonction renvoie le status de son opération, qui peut être :
     * - ERROR_INTERRUPTED : le thread a été interrompu pendant qu'il attendait son tour
    ...
    public OperationStatus delete() {
        synchronized (this) {
            if( state == State.MARKED_FOR_DELETION ) {
                System.err.println("oneServerMultiClient.comuneCode.FileHandle : double delete on file " + file.getName());
                return OperationStatus.OK;
            }
